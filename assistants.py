...
class BasicAssistant:
    ...
    def _prepare_intents_data(self, ignore_letters: tuple = ("!", "?", ",", ".")):
        ...
        for document in documents:
            ...
            pattern_words = [self.lemmatizer.lemmatize(
                w.lower()) for w in pattern_words]
    ...
    def fit_model(self, optimizer: Optimizer = None, epochs: int = 200):
        ...
        self.history = self.model.fit(
            X, y, epochs=epochs, batch_size=5, verbose=1)
    ...
    def load_model(self):
        ...
        self.intents = pickle.load(
            open(f'{self.model_name}_intents.pkl', 'rb'))

    def _predict_intent(self, input_text: str):
        input_words = nltk.word_tokenize(input_text)
        input_words = [self.lemmatizer.lemmatize(
...
class GenericAssistant(BasicAssistant):
    def __init__(self, *args, **kwargs):
        import warnings
        warnings.warn("The 'GenericAssistant' class is deprecated and will be removed in future versions. Please use 'BasicAssistant' instead.",
